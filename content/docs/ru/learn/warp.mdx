...
<Callout type="info">
  **Конфигурация Docker для Xray WireGuard Outbound:**
  Если вы запускаете PasarGuard Node через Docker, добавьте следующие строки в файл `docker-compose.yml` для корректной работы WireGuard outbound:
